...
import dev.company.service.AddressServiceImpl;
import dev.company.service.CompanyServiceImpl;
...
public class ServiceLoggingAdvise<T> {
	...
	private final Logger companyServiceLogger = LoggerFactory.getLogger(CompanyServiceImpl.class);

	/*
	 * no service methods exist for this service yet hence no need to create custom
	 * pointcuts
	 */
	private final Logger addressServiceLogger = LoggerFactory.getLogger(AddressServiceImpl.class);
	...
	public void afterServiceExceptionCallForCreate(Throwable throwingException) {
		companyServiceLogger.error("an exception occured when creating the resource -" + throwingException);
	}
	...
	public void afterServiceExceptionCallForRead(Throwable throwingException) {
		companyServiceLogger.error("an exception occured when reading the resource -" + throwingException);
	}
	...
	public void afterServiceExceptionCallForUpdate(Throwable throwingException) {
		companyServiceLogger.error("an exception occured when updating the resource -" + throwingException);
	}
	...
	public void afterServiceExceptionCallForDelete(Throwable throwingException) {
		companyServiceLogger.error("an exception occured when deleting the resource -" + throwingException);
	}
	...
	public T aroundServiceMethodCall(ProceedingJoinPoint jp) throws Throwable {
		...
		companyServiceLogger.info(methodName + " method with " + argSize + " arg(s) call start.");

		T result = (T) jp.proceed();

		companyServiceLogger.info(methodName + " method with " + argSize + " arg(s) call end.");
